<template>
  <Primitive v-bind="props" class="group/EditableWrapper relative">
    <button
      v-if="allowed"
      class="absolute top-2 right-2 flex items-center gap-1.5 text-gray-200/60 hover:text-gray-200 opacity-0 group-hover/EditableWrapper:opacity-100 transition-all z-20"
      @click="value = !value"
    >
